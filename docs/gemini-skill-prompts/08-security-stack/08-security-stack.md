...
Structure your response with these EXACT top-level headers (using `#`) so it can be split into four separate skill files. Each section must be self-contained — do not assume the reader has read the other sections.
